...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oieee @${num.split('@')[0]}\Bem vindo(a) ao grupo *${mdata.subject}* seja legal com todos e leia as regras, e o mais importante, siga todas elas para que não seja banido do grupo, sinta-se em casa😊💜`
				...
				teks = `Finalmente a carga do grupo foi reduzida , tchau🙃💜 @${num.split('@')[0]} Seus serviços serão enterrados profundamente²`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
        ...
        if (levelRole <= 3) {
           ...
           return false
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
                ...
                if (namaUser.length >= 30) return reply(`Por quê seu nome é tão longo isso é um nome ou um trem?`)
          ...
          case 'ping':
          if (!isRegistered) return reply(ind.noregis())
            await client.sendMessage(from, `Pong!!!!\nVelocidade: ${processTime(time, moment())} _Second_`)
					break
               case 'help': 
				case 'menu11':
				if (!isRegistered) return reply(ind.noregis())
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return client.sendMessage(from, 'Idioma necessário!', text, {quoted: mek})
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('O texto é principalmente tio ')
           ...
           case 'demote':
				...
				if (!isQuotedSticker) return reply('Marque a figurinha!')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Convertida com successo✅ '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('𝘀𝘂𝗱𝗮𝗵 ??𝗸𝘁𝗶𝗳 !!!')
						samih.push(from)
