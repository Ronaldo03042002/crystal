...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Ronaldo\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=557196834782:+55 71 99683-4782\n' 
            + 'END:VCARD' 
blocked = []   
prefix = '#'
limitawal = 9999999
...
/******** OWNER NUMBER**********/
const ownerNumber = ["557196834782@s.whatsapp.net","7186202988@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
        ...
        if (levelRole <= 3) {
          ...
          const checkLimit = (sender) => {
           ...
           return false
			...
			switch(command) {
				...
				teks = `*「 Música encontrada 」*\n\n*Título* : ${anu.result.judul}\n*Álbum* : ${anu.result.album}\n*publicado em* : ${anu.result.dipublikasi}\n*Letra da música* : ${anu.result.lirik}`
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
                 infomp3 = `*Sua música foi encontrada✅*\nAutoria : ${data.result.judul}\nNome da música : ${data.result.album}\nLançada em : ${data.result.dipublikasi}`
				...
				case 'bcgc':
					...
					client.sendMessage(from, pok, image, { quoted: mek, caption: `*Prontinho✅*`})
					await limitAdd(sender)
					break 
					case 'resepmasakan':
					if (!isRegistered) return reply(ind.noregis())
                ...
                if (namaUser.length >= 30) return reply(`Seu nome é tão longo é um nome ou um trem?`)
                if (umurUser.length >= 3, umurUser.length <= 1) return reply(`Sua idade é muito jovem / idoso - idade mínima de 10 anos e máxima de 40 anos`)
                veri = sender
                if (isGroup) {
            	...
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} A mineração de eventos não foi ativada pelo proprietário`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`Você é nosso proprietário da equipe bot de envio ${one}Xp para você`)
                      }else{
          ...
          case 'ping':
          if (!isRegistered) return reply(ind.noregis())
            await client.sendMessage(from, `\nRapidez: ${processTime(time, moment())} _Segundos_`)
					break
               case 'coman1dos': 
				case 'me1nu':
				if (!isRegistered) return reply(ind.noregis())
               ...
               client.sendMessage(from, resul, text, { quoted: mek})
				...
				case 'info':
					...
					break
				case 'blocklist': 
					teks = 'Lista de pessoas bloqueadas :\n'
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'limpar':
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(from, `Pedido recebido, bloqueado com sucesso✅${body.slice(7)}@c.us`, text)
					break
                    case 'unblock':
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
				    client.blockUser (`${body.slice(9)}@c.us`, "remove")
					client.sendMessage(from, `Membro desbloqueado com sucesso✅ ${body.slice(9)}@c.us`, text)
					break
				case 'leave': 
				if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
				await reply(from, 'Bye pessoal').then(() => client.groupLeave(groupId))
					break
				case 'bc': 
					if (!isOwner) return reply(ind.ownerb()) 
					if (args.length < 1) return reply('.......')
			   	...
			   	case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('Foto de perfil atualizada com sucesso✅')
					break						
				case 'add':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
            ...
            case 'dono':
            case 'criador':
                  client.sendMessage(from, {displayname: "Ronaldo", vcard: vcard}, MessageType.contact, { quoted: mek})
                  client.sendMessage(from, 'Este é o número do meu proprietário, não envie spam ou eu te bloqueio',MessageType.text, { quoted: mek} )
					break    
           case 'setname':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
           ...
           case 'rebaixar':
				...
				case 'promover':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Promovido a administrador do grupo com sucesso✅ :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Parabéns @${mentioned[0].split('@')[0]} *você se tornou o administrador do grupo* `, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'ban':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque a pessoa que você deseja banir!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
				...
				case 'listadmin':
					if (!isGroup) return reply(ind.groupo())
					teks = `Lista de administradores *${groupMetadata.subject}*\nQuantidade : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'toimg':
				if (!isRegistered) return reply(ind.noregis())
				if (!isQuotedSticker) return reply('Marque a figurinha que você deseja converter!')
					reply(ind.wait())
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Convertida com sucesso✅ '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ 𝗠??𝗻??𝗮??𝘁𝗶𝗳𝗸𝗮𝗻 𝗳𝗶𝘁𝘂𝗿 𝘀𝗶𝗺𝗶 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽 𝗶𝗻𝗶️')
				...
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('Recurso já ativado, preste atenção!')
					} else {
						reply(ind.satukos())
					}
					break
                ...
                if (args[0] === 'habilitar') {
                 ...
                 case 'block':
					...
					client.sendMessage(from, `Pedido recebido, bloquear wa.me${body.slice(8)}@c.us`, text)
					break
				case 'unblock':
					client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, `Pedido recebido, desbloquear wa.me/${body.slice(10)}`, text)
					break
				case 'welcome':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
			...
			if (body.startsWith(`${prefix}${command}`)) {
                  ...
                  reply(`Desculpe *${pushname}*, para usar o comando *${prefix}${command}* você precisa se registrar no banco de dados do bot!`)
